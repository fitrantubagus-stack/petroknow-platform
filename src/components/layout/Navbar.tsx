...
export const Navbar: React.FC = () => {
  ...
  return (
    <header className="sticky top-0 z-40 w-full bg-slate-900/95 backdrop-blur-md border-b border-slate-800 text-slate-100 flex flex-col justify-center">
      <div className="h-16 w-full flex items-center justify-between px-2.5 sm:px-6 min-w-0">
        ...
        <div className="flex items-center gap-1.5 sm:gap-4 min-w-0">
          ...
          {currentView !== 'landing' && (
            <button
              ...
              className="md:hidden p-2 rounded-xl bg-slate-800/80 hover:bg-slate-800 text-slate-300 border border-slate-700/60 transition-colors shrink-0"
            >
              {mobileSidebarOpen ? <X className="w-5 h-5 text-teal-400" /> : <Menu className="w-5 h-5" />}
            </button>
          )}
          ...
          >
            ...
            <div className="flex flex-col min-w-0">
              <div className="flex items-center gap-1 sm:gap-1.5">
                <span className="text-sm sm:text-base font-extrabold tracking-tight text-slate-100 font-sans truncate">
                  Petro<span className="text-teal-400">Know</span>
                </span>
                <span className="hidden sm:inline-block text-[9px] sm:text-[10px] font-mono font-bold px-1.5 py-0.2 rounded bg-slate-800 text-teal-400 border border-slate-700 shrink-0">
                  PROTOTYPE
                </span>
              </div>
            ...
            <div className="flex items-center pl-1.5 sm:pl-3 border-l border-slate-800 shrink-0">
              <ChandraAsriLogo size={20} showWordmark={true} wordmarkClassName="hidden sm:flex" />
            </div>
          </Link>
          ...
          {currentView !== 'landing' && (
            <Link
              to="/"
              onClick={() => setCurrentView('landing')}
              className="hidden md:flex items-center gap-1 text-xs text-slate-400 hover:text-slate-200 px-2 py-1 rounded-md hover:bg-slate-800/60 transition-colors shrink-0"
            >
        ...
        {/* Right Controls: Mobile Search Toggle, Role Switcher, Alerts, Reset & User */}
        <div className="flex items-center gap-1.5 sm:gap-3 shrink-0">
          ...
          {canInstallPwa && (
            <button
              onClick={installPwa}
              title="Install PetroKnow as standalone app"
              className="hidden sm:flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-teal-500/20 hover:bg-teal-500/30 text-teal-300 border border-teal-500/40 text-xs font-semibold transition-all shadow-sm shrink-0"
            >
          ...
          {/* PWA Install Button (Mobile Icon) */}
          {canInstallPwa && (
            <button
              onClick={installPwa}
              title="Install PetroKnow App"
              aria-label="Install PetroKnow App"
              className="sm:hidden p-2 rounded-xl bg-teal-500/20 hover:bg-teal-500/30 text-teal-300 border border-teal-500/40 transition-colors shrink-0"
            >
          ...
          <div className="relative" ref={notifRef}>
            ...
            {notificationsOpen && (
              <div className="absolute right-0 mt-2 w-72 sm:w-80 max-w-[calc(100vw-1.5rem)] bg-slate-900 border border-slate-700 rounded-xl shadow-2xl p-3 z-50 animate-fade-in text-xs space-y-2">
          ...
          <div className="relative" ref={dropdownRef}>
            <button
              ...
              className={`flex items-center gap-1.5 sm:gap-2 px-2 sm:px-3 py-1.5 rounded-xl border transition-all ${roleColors[role].badge}`}
            >
              {getRoleIcon(role)}
            ...
            {roleDropdownOpen && (
              <div className="absolute right-0 mt-2 w-64 max-w-[calc(100vw-1.5rem)] bg-slate-900 border border-slate-700/90 rounded-xl shadow-2xl p-2 z-50 animate-fade-in text-xs space-y-1">
      ...
      {mobileSearchOpen && currentView !== 'landing' && (
        <div className="md:hidden px-3 sm:px-4 pb-3 pt-1 border-t border-slate-800/80 bg-slate-900 animate-fade-in">
